...
public:
    ...
    FloppyMesh(std::string meshPath,
        glm::vec3 initialTranslation = glm::vec3(0.0f),
        float initialScale = 1.0f,
        float initialRotation = 0.0f,
        float subsequentRotationSpeed = 0.0f
        );
...
protected:
    ...
    float _subsequentRotation{};  /**< The subsequent rotation around the Y-axis applied to the mesh. Mostly for debugging */
    float _subsequentRotationSpeed;     /**< The subsequent rotation speed around the Y-axis applied to the mesh. Mostly for debugging */
     ...
     */
    static bool loadObj(const std::string& filename, uint partIndex,
     ...
     */
    static GLuint loadTexture(std::string path, TextureType type = SRGB);
